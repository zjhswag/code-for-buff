...
if(textContains("受限").exists())//915
{
    ...
    else if(textContains("响尾蛇").exists()||textContains("） | 金铋辉煌").exists()||textContains("不可磨灭").exists()||textContains("） | 抖枪").exists()||textContains("枪 | 重新启动").exists()||textContains("） | 水蓝条纹").exists()||textContains("病毒危机").exists()||textContains("MAG-7（StatTrak™） | 炽热").exists()||textContains("0 | 销声").exists()||textContains("） | 酸蚀").exists()||textContains("牛 | 路霸").exists()||textContains("星 | 一见青心").exists()||textContains("塔 | 灾难").exists()||textContains("） | 蚀刻领主").exists()||textContains("） | 销声").exists()||textContains("MAC-10（StatTrak™） | 炽热").exists()||textContains("型 | 鼬鼠").exists()||textContains("） | 黯翼").exists()||textContains("型 | 氮化处理").exists())
    {
        buy008();
        toast('00 07-08');
        breakbuy();
