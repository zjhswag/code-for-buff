...
if(textContains("受限").exists())//915
{
    ...
    else if(textContains("） | 隐伏帝王龙").exists()||textContains("诅咒").exists()||textContains("） | 古董枪").exists()||textContains("） | 翼蜥").exists()||textContains("铁血玫瑰").exists()||textContains("牛 | 古董枪").exists()||textContains("） | 杀意大名").exists()||textContains("7 | 轨道").exists()||textContains("冥界之").exists()||textContains("林间").exists()||textContains("P | 火卫一").exists()||textContains("盲点").exists()||textContains("0 | 死亡之握").exists()||textContains("） | 蓝色层压板").exists()||textContains("星 | 惊惧骷髅").exists()||textContains("次时代").exists()||textContains("数字架构师").exists()||textContains("棱彩阶梯").exists()||textContains("斯 | 电子脉冲").exists()||textContains("） | 西装革履").exists()||textContains("） | 帝国").exists()||textContains("P | 蠕虫之神").exists()||textContains("8 | 浮生如梦").exists()||textContains("4 | 杀意大名").exists()||textContains("版 | 地狱门票").exists()||textContains("） | 富士山").exists()||textContains("） | 深红海啸").exists())//||textContains("） | 崩络克").exists()||textContains("4 | 西装革履").exists()||textContains("4 | 特克卢").exists()||textContains("型 | 夜无眠").exists()
    {
       buy001();  
       toast('00-01 啊');
       breakbuy();      
    }
     else if(textContains("血红蛋白").exists()||textContains("射线").exists()||textContains("金砖").exists()||textContains("幕后主谋").exists()||textContains("犯罪现场").exists()||textContains("4 | 狮鹫").exists()||textContains("花纹钢板").exists()||textContains("铜色星系").exists()||textContains("钛片").exists()||textContains("错觉").exists()||textContains("橙红安").exists()||textContains("） | 地狱门票").exists()||textContains("红色 DD").exists()||textContains("7 | 海之泡沫").exists()||textContains("） | 轻轨").exists()||textContains("鹰 | 轻轨").exists()||textContains("夜空沙暴").exists()||textContains("雅藤如嫣").exists()||textContains("） | 狮鹫").exists()||textContains("） | 火卫一").exists()||textContains("） | 钢铁禁锢").exists()||textContains("富兰克林").exists()||textContains("烧尽").exists()||textContains("遗产").exists()||textContains("0 | 翡翠色调").exists()||textContains("型 | 翼蜥").exists()||textContains("地狱看门犬").exists()||textContains("） | 蠕虫之神").exists()||textContains("太古传说").exists()||textContains("远古幻想").exists()||textContains("鬼脸天蛾").exists()||textContains("） | 祥和之翼").exists()||textContains("） | 猫猫狗狗").exists()||textContains("） | 夜无眠").exists()||textContains("型 | 隐伏帝王龙").exists()||textContains("型 | 核子花园").exists()||textContains("钴蓝石英").exists()||textContains("9 | 晶红石英").exists()||textContains("型（StatTrak™） | 城里的月光").exists()||textContains("） | 浮生如梦").exists())
     ...
     else if(textContains("型 | 崩络克").exists()||textContains("牛 | 水蓝条纹").exists()||textContains("） | 海斗士").exists()||textContains("塔 | 海斗士").exists()||textContains("） | 皇家军团").exists()||textContains("型 | 皇家军团").exists()||textContains("指挥").exists()||textContains("8 | 幽灵战士").exists()||textContains("塔 | 皇室伴侣").exists()||textContains("0 | 炽热").exists()||textContains("） | 探员").exists()||textContains("） | 手炮").exists()||textContains("P | 亡灵之主").exists()||textContains("） | 冷石").exists()||textContains("R | 冷石").exists()||textContains("型 | 闪回").exists()||textContains("斯 | ZX81").exists()||textContains("） | 圆影").exists()||textContains("消音型（StatTrak™） | 闪回").exists()||textContains("彼岸花").exists()||textContains("P | 猫猫狗狗").exists()||textContains("7 | 复古浪潮").exists()||textContains("4 | 星级").exists()||textContains("7 | 三位一体").exists()||textContains("型 | 氮化处理").exists()||textContains("） | 火线冲锋").exists())//||textContains("7 | 墨岩").exists()|textContains("） | 猫猫狗狗").exists()
     //只买0.07 和0.00  ||textContains("） | 速递").exists()||textContains("鹰 | 后发制人").exists()
     {
         toast('00-07');         
         breakbuy();
     }
    else if(textContains("） | 食人花").exists()||textContains("响尾蛇").exists()||textContains("） | 金铋辉煌").exists()||textContains("不可磨灭").exists()||textContains("） | 抖枪").exists()||textContains("枪 | 重新启动").exists()||textContains("） | 水蓝条纹").exists()||textContains("病毒危机").exists()||textContains("MAG-7（StatTrak™） | 炽热").exists()||textContains("0 | 销声").exists()||textContains("） | 酸蚀").exists()||textContains("牛 | 路霸").exists()||textContains("星 | 一见青心").exists()||textContains("塔 | 灾难").exists()||textContains("） | 销声").exists()||textContains("MAC-10（StatTrak™） | 炽热").exists()||textContains("型 | 鼬鼠").exists()||textContains("） | 黯翼").exists())
    {
        buy008();
        toast('00 07-08');
        breakbuy();
    ...
    else if(textContains("7 | 北冥有鱼").exists()||textContains("5 | 猛虎").exists()||textContains("9 | 翠绿箭").exists()||textContains("4 | 四季").exists()||textContains("0 | 绝界之行").exists()||textContains("5 | 支架").exists()||textContains("） | 路霸").exists()||textContains("守护者").exists()||textContains("0 | 孔雀石").exists()||textContains("0 | 翼击").exists()||textContains("） | 七彩斑斓").exists()||textContains("7 | 七彩斑斓").exists()||textContains("5 | 金铋辉煌").exists()||textContains("8 | 抖枪").exists()||textContains("公路杀手").exists()||textContains("R | 深红海啸").exists()||textContains("0 | 权力之心").exists()||textContains("） | 翼击").exists()||textContains("） | 树蝰").exists()||textContains("） | 经济").exists()||textContains("3 | 三巨头").exists()||textContains("） | 红宝石毒").exists()||textContains("） | 重新启动").exists()||textContains("） | 贵族").exists()||textContains("7 | 渐变之色").exists()||textContains("7 | 翡翠细条纹").exists())//||||textContains("R | 火线冲锋").exists()||textContains("4 | 蚀刻领主").exists()||textContains("型 | 崩络克").exists()||textContains("枪 | 模拟输入").exists()||textContains("7 | 笑一个").exists()||textContains("7 | 混合体").exists()||textContains("） | 皇室伴侣").exists()
    {
        buy008();
        toast('00-01,07-08');
        breakbuy();
    ...
    else if(textContains("SG 553（StatTrak™） | 电子脉冲").exists()||textContains("3 | 电子脉冲").exists()||textContains("核子威慑").exists()||textContains("） | 中士").exists()||textContains("澄澈之水").exists()||textContains("） | 笑一个").exists()||textContains("） | 模拟输入").exists()||textContains("） | 混合体").exists()||textContains("） | 蚀刻领主").exists()) //
         {
            buy008();
            buy009();
           buy010();
